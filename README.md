...
完整的資產盤點、對手分類、各元件控制對映與 non-goals，見 `THREAT_MODEL.md`。下面是摘要；細節以該文件為準，漏洞通報流程則見 `SECURITY.md`。

本專案的安全設計定位是「威懾、追蹤、降低低階誤用摩擦」，不是「防止外洩」。靜態前端只要把資料送到瀏覽器，就不能保證阻止使用者複製、截圖、攔截或自動化讀取。瀏覽器端控制不是 authentication，也不是 authorization；不可公開的資料不應放進靜態前端 bundle。
